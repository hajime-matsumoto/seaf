...
class Route
{
    ...
    public function match (Request $request)
    {
        ...
        if ($this->methods !== array('*')) {
            if (!count(array_intersect(array($uri->method(), '*'),$this->methods)) > 0){
                return false;
            }
        }

        if( $this->pattern === "*" || $this->pattern === $uri->path() ) return true;
        ...
        // * で指定されているパターン位置を取得
        $splat = substr($uri->path(), strpos($this->pattern, '*'));
        ...
        if( preg_match('#^'.$regex.'(?:\?.*)?$#i', $uri->path(), $m) ) {
            $params = array();
