...
class FileSystem implements KeyValueStore\KVSComponentIF
{
    ...
    public function __construct($cfg)
    {
        ...
        $this->setFileSystem($f = new FSHandler(
            $cfg(
                'rootPath',
                Registry\Registry::getSingleton()->getVar('kvs_file_dir', '/tmp')
            )
        ));
    }
    ...
    {
        $status = unserialize($this->getFileSystem( )->fileGetContents($table.'/'.sha1($key).'_status'));
        return unserialize($this->getFileSystem( )->fileGetContents($table.'/'.sha1($key)));
    }
    ...
    public function set ($table, $key, $data, $status = [])
    {
        ...
        $this->getFileSystem( )->filePutContents($table.'/'.sha1($key), serialize($data));
        $this->getFileSystem( )->filePutContents($table.'/'.sha1($key).'_status', serialize($status));
    }

    public function status($table, $key)
    {
        return unserialize($this->getFileSystem( )->fileGetContents($table.'/'.sha1($key).'_status'));
    }

    public function has ($table, $key)
    {
        return $this->getFileSystem( )->fileExists($table.'/'.sha1($key));
    }
