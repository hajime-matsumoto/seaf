...
class Handler extends Database\DatabaseHandler
{
    ...
    public function query ($query)
    {
        ...
        if (!$result) {
            $this->warn('MYSQL|QUERY_ERROR',
                sprintf("[%s] %s", $query, $this->getLastError()
            ));
        }else{
            $this->debug('MYSQL|QUERY_OK', $query);
        }
     ...
     * @param array
     */
    public function findTable($array)
    {
        $findQuery = Util::ArrayContainer($array);

        $sql = 'SELECT';

        $sql.= ' '.$this->buildFields($findQuery('fields'));
        $sql.= ' FROM `';
        $sql.= $this->escapeVars($findQuery('name'));
        $sql.= '`';
    ...
    public function createTable($array)
    {
        ...
        $cfg = Util::ArrayContainer($array);

        if ($cfg->dict('options')->get('useDrop',false)) {
            $sql = sprintf('DROP TABLE IF EXISTS `%s`;', $esc($cfg('name')));
            $this->query($sql);
        }

        $sql = sprintf('CREATE TABLE IF NOT EXISTS `%s`', $esc($cfg('name')));
        $sql.= " (\n";
        ...
        if ($cfg('primary_index',false)) {
            $sql = sprintf("ALTER TABLE `%s`", $esc($cfg('name')));
