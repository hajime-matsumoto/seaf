...
class SeafWebTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testWebExtendsClass ( )
    {
        ...
        Seaf::Web( )
            ...
            ->mount('/assets/', 'Seaf\Application\AssetManager\Base');

        Kernel::Globals( )->set('SERVER', array(
            'REQUEST_URI'                 => '/app/home',
            'SCRIPT_NAME'                 => '/app/index.php',
        ));
        Seaf::Web()->request()->initRequest();

        Seaf::Web()->run();
        $this->assertRegExp(
            '#<h1>Seaf</h1>#',
            Seaf::Web()->response()->body
        );
        Seaf::Web()->response()->init();

        Kernel::Globals( )->set('SERVER', array(
            'REQUEST_URI'                 => '/app/assets/styles/app.css',
            'SCRIPT_NAME'                 => '/app/index.php',
        ));
        Seaf::Web()->request()->initRequest();
        Seaf::Web()->run();
        ...
        Kernel::Globals( )->set('SERVER', array(
            'REQUEST_URI'                 => '/app/assets/scripts/app.js',
            'SCRIPT_NAME'                 => '/app/index.php',
        ));
        Seaf::Web()->request()->initRequest();
        Seaf::Web()->run();
